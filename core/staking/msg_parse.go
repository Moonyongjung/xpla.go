...
func parseCreateValidatorArgs(
	...
	privKeyAddr := util.GetAddrByPrivKey(privKey)
	...
	if privKeyValAddr.String() != addr.String() {
		return nil, util.LogErr("CreateValidatorMsg.ValidatorAddress and validator address generated by using private key are not same")
	}
...
func initializedNodeValidatorString(nodeKey string, privValKey string) (string, cryptotypes.PubKey, error) {
	...
	if err != nil {
		return "", nil, util.LogErr("err reading privValidator key, ", err)
	}
