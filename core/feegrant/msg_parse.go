...
func parseFeeGrantArgs(feeGrantMsg types.FeeGrantMsg, privKey key.PrivateKey) (feegrant.MsgGrantAllowance, error) {
	...
	if feeGrantMsg.Granter != granter.String() {
		return feegrant.MsgGrantAllowance{}, util.LogErr("Account address generated by private key is not equal input granter of msg")
	}
	...
	if periodClock > 0 || feeGrantMsg.PeriodLimit != "" {
		...
		if periodClock <= 0 {
			return feegrant.MsgGrantAllowance{}, util.LogErr("period clock was not set")
		}

		if periodLimit == nil {
			return feegrant.MsgGrantAllowance{}, util.LogErr("period limit was not set")
		}

		periodReset := getPeriodReset(periodClock)
		if feeGrantMsg.Expiration != "" && periodReset.Sub(expireTime) > 0 {
			return feegrant.MsgGrantAllowance{}, util.LogErr("period (", periodClock, ") cannot reset after expiration (", feeGrantMsg.Expiration, ")")
		}
