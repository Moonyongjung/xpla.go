...
func (s *IntegrationTestSuite) TestListContractByCode() {
	for i, api := range s.apis {
		...
		if i == 0 {
			...
			s.T().Log(res)
			s.T().Log(queryContractsByCodeResponse)
...
func (s *IntegrationTestSuite) TestContractHistory() {
	for i, api := range s.apis {
		...
		s.T().Log(res)
		s.T().Log(util.FromStringToUint64(s.wasmCodeID), queryContractHistoryResponse.Entries[0].CodeID)
