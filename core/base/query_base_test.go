...
func (s *IntegrationTestSuite) TestValidatorSet() {
	...
	for i, api := range s.apis {
		...
		s.xplac.EncodingConfig.InterfaceRegistry.UnpackAny(getLatestValidatorSet.Validators[0].PubKey, &pubkey)
		...
		s.xplac.EncodingConfig.InterfaceRegistry.UnpackAny(getValidatorSetByHeightResponse.Validators[0].PubKey, &pubkey)
